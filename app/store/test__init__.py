...
def test_resolved_placements_query():
    assert strip_lines(PlacementsQuery.resolved.placements()) == snapshot(
        """\
SELECT placements.placement_id, unixepoch(placements.placed_at) AS placed_at, unixepoch(placements.canceled_at) AS canceled_at, unixepoch(placements.completed_at) AS completed_at, placed_items.product_id, count(placed_items.product_id) AS count, products.name, products.filename, products.price
FROM placements JOIN placed_items ON placements.placement_id = placed_items.placement_id JOIN products ON products.product_id = placed_items.product_id
